            ...
            buttons: [
                {
                    ...
                    'click': function (e) {
                     ...
                     //    	$(obj).data('handle','1');
                        ...
                        window.location.href = domain + '/carmortgage/swap/init';
		            	// window.location.reload();
...
$(function () {
    ...
    (function init() {
        ...
        removeCar();
        // 提交汽车信息
        $('.submit-btn').on('click', submitCarInfo);
				                ...
				                '<ul class="info-wrap">'+
                                   ...
                                   /* '<li class="car-img-wrap clearfix">'+
				                    ...
				                    '<li class="listrow btn-row">'+
				                        '<a href="javascript:;" class="btn del-btn">删除</a>'+
				                    '</li>'+
				                '</ul>'+
				            '</li>';
	...
	function removeCar() {
		$('.car-info-all').on('click', '.del-btn', function(){
			$(this).closest('.auto-info-item-edit').remove();
		})
	}
	...
	function submitCarInfo() {
		...
		oData = 'orderId=' + $('#global_orderId').val()+'&comment=' + $('#exampletextarea').val() + '&userId=' + $('#global_userId').val() + '&serialNos=' + sCarId + '&jsonData=[' + String(carInfoArry) + "]" ;
		
		if(checkNum('.mileage') && checkNum('.price') && checkNum('.assess')) {
	        $.ajax({
	            url: domain + '/carmortgage/swap/init/submit',
	            type: 'post',
	            data: oData,
	            dataType: 'json',
	            async: true,
